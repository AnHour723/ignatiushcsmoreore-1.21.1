...
public class DisplayStandEntityRenderer implements BlockEntityRenderer<DisplayStandEntity> {
    public DisplayStandEntityRenderer(BlockEntityRendererProvider.Context context){

    }

    @Override
    public void render(DisplayStandEntity displayStandEntity, float v, PoseStack poseStack,
        ...
        ItemStack stack = displayStandEntity.inventory.getStackInSlot(0);// 要渲染的物品来自于方块实体里的库存 槽位为0的位置

        poseStack.pushPose();// 推送到后栈  -->  表面我们要渲染某个东西  (开始记录要渲染的东西)
        poseStack.translate(0.5f,1.15f,0.5f);// 渲染位置
        poseStack.scale(0.3f,0.3f,0.3f);// 缩放比例
        // 旋转 旋转轴体为Y轴 rotationDegrees --> 输入我们想旋转的角度
        poseStack.mulPose(Axis.YP.rotationDegrees(displayStandEntity.getRenderingRotation()));

        // 使用 renderStatic 方法直接进行渲染
        itemRenderer.renderStatic(stack, ItemDisplayContext.FIXED,// FIXED --> 为如同物品掉落物一样渲染
                getLightLevel(displayStandEntity.getLevel(), displayStandEntity.getBlockPos()),
                OverlayTexture.NO_OVERLAY,poseStack,multiBufferSource,displayStandEntity.getLevel(),1);
