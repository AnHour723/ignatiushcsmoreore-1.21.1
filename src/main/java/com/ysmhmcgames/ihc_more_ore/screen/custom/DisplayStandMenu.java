...
public class DisplayStandMenu extends AbstractContainerMenu {
    public final DisplayStandEntity blockEntity;
    ...
    public DisplayStandMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        // 传入容器id 传入储存槽 再传入储存槽->通过玩家、层级、获取方块实体并传入额外数据获取方块位置
        this(containerId,inv,inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }
    public DisplayStandMenu(int containerId, Inventory inv, BlockEntity blockEntity) {// 创建下一层级的构造方法
        super(HCMenuTypes.DISPLAY_STAND_MENU.get(), containerId);
        this.blockEntity = ((DisplayStandEntity)blockEntity);
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = 1;// 自定义方块实体的库存槽 (可根据自定义库存槽数量更改)
    @Override
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, HCBlock.DISPLAY_STAND.get());
    }
